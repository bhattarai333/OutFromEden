...
public class StateThree extends GameState
{
    ...
    public void init(GameStateManager gms){
        ...
        objects.set(d-1,new Terra(
                boundingBoxes,tm,200,32,0//add moose enemy at 70 32
        ));
        objects.get(d-1).dialogue = "Moose is causing trouble outside the hospital,\ncould you go take care of him?";
        d++;
        terraVal = d-2;
        if(d>objects.size()){
            objects.add(new Obj());
        ...
        objects.set(d-1,new Door(
                boundingBoxes,tm,310,530,1,"doorSheet"
        ));
    ...
    public void objectHandling(){
        for(int i = 0; i < objects.size();i++){
            ...
            if(objects.get(i).speak){
                ...
                if(i==terraVal && gsm.go.mooseKills>0){
                    roy1.health = 50;
            ...
            if(objects.get(i).enemy&&roy1.rect.intersects(objects.get(i).nextRect)
                ...
                roy1.hit(1,objects.get(i).x);
            }


            if(gsm.go.mooseKills>0){
                objects.get(terraVal).dialogue = "Thanks for killing moose, let me heal you up.";
            }
        }
